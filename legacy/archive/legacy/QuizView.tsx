...
const QuizProgressPanel: React.FC<QuizProgressPanelProps> = ({
  ...
  const correctAnswers = Object.entries(userAnswers).filter(
    ([questionId, selectedOption]) => {
      ...
      return question?.options.find((o) => o.text === selectedOption)
        ?.isCorrect;
    },
  ).length;
    ...
    <div className="bg-white p-4 rounded-lg shadow-md">
      ...
      <div className="space-y-2 text-sm mt-4 pt-4 border-t border-gray-200">
        <div className="flex items-center text-green-600">
          ...
          <span className="font-bold ml-1">{correctAnswers}</span>
        </div>
        <div className="flex items-center text-red-600">
          <XCircleIcon className="w-5 h-5 mr-2" /> Incorrect:{" "}
          <span className="font-bold ml-1">{incorrectAnswers}</span>
        </div>
        <div className="flex items-center text-gray-600">
          <FlagIcon className="w-5 h-5 mr-2" /> Flagged:{" "}
          <span className="font-bold ml-1">{flaggedQuestions.length}</span>
        </div>
      </div>
      <button
...
const QuizView: React.FC<QuizViewProps> = ({
  ...
  useEffect(() => {
    ...
    let unseenQuestions = potentialQuestions.filter(
  ...
  const handleFinishTest = (timeExpired = false) => {
    const finalUserAnswers: UserAnswer[] = questions.map((q) => {
      ...
      const isCorrect =
        q.options.find((o) => o.text === selectedOption)?.isCorrect || false;
    ...
    onQuizComplete({
      results: Object.entries(results).map(([cat, res]) => ({
  ...
  const selectedOptionForCurrent = currentQuestion
    ? answers[`${currentQuestion.id}`]?.choice || null
    : null;
        ...
        <div className="w-full flex justify-between items-center text-sm">
          ...
          <div className="flex space-x-2">
            ...
            >
              ...
              <span>
                {isCurrentQuestionFlagged ? "Flagged" : "Flag for Review"}
              </span>
            </button>
          </div>
        </div>
