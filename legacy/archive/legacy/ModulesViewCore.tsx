  ...
  lines.forEach((line, index) => {
    ...
    if (t.startsWith("# ")) {
      elements.push(
        <h2
          key={`h1-${index}`}
          className="text-2xl font-bold mt-6 mb-3 text-gray-900 border-b pb-1"
        >
          {t.substring(2)}
        </h2>,
      );
    ...
    elements.push(
      <p key={`p-${index}`} className="my-3 text-gray-700 leading-relaxed">
        ...
        {submitted ? (
          ...
          <button
            ...
            onClick={() => {
              if (!selected) return;
              const isCorrect =
                questions[index].options.find((o) => o.text === selected)
                  ?.isCorrect || false;
              setAnswers((prev) => [
                ...prev,
                {
...
const ModulesViewCore: React.FC<ModulesViewProps> = ({
  selectedModule,
  setSelectedModule,
  latestQuizResults,
  onModuleMastery,
  ...
  if (selectedModule) {
    ...
    return (
      <ErrorBoundary>
        <div
          className="bg-white p-6 md:p-8 rounded-lg shadow-md max-w-4xl mx-auto space-y-4"
          data-build-version="modules-core-detail-title-1"
        >
          <button
    ...
    try {
      return LEARNING_MODULES.filter(
        (m) =>
          ...
          assertString("module.category", m.category)
            .toLowerCase()
            .includes(q) ||
  ...
  return (
    <ErrorBoundary>
      <div
        ...
        data-build-version="modules-core-v2-search-1"
      >
