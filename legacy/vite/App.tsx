...
const App: React.FC = () => {
  ...
  const [chatContextMessage, setChatContextMessage] = useState<string | null>(
    null,
  );
  ...
  const [progress, setProgress] = useState<QuizResult[]>(() => {
    try {
      ...
      return savedProgress
        ...
        : DVSA_CATEGORIES.map((cat) => ({
            category: cat,
            correct: 0,
            total: 0,
          }));
    } catch (e) {
      return DVSA_CATEGORIES.map((cat) => ({
        category: cat,
        correct: 0,
        total: 0,
      }));
  ...
  const handleQuizComplete = useCallback(
    (summary: Omit<FinalQuizResults, "pointsEarned">) => {
      const percentage =
  ...
  const handleModuleMastery = useCallback(
        ...
        return (
          <SupaAuthGate>
            <QuizView
              ...
              onQuestionsSeen={handleQuestionsSeen}
            />
          </SupaAuthGate>
        );
      case "quiz-results":
    ...
    <ErrorBoundary
      fallback={
        <div className="p-6 m-6 bg-red-50 border border-red-200 rounded">
          The app encountered an error while rendering. Please reload.
        </div>
      }
    >
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="fixed bottom-6 right-6 z-50">
          ...
          >
            <div
              className={`transition-transform duration-300 ease-in-out absolute ${isChatWidgetOpen ? "rotate-180 scale-0" : "rotate-0 scale-100"}`}
            >
              <ChatIcon className="w-8 h-8" />
            </div>
            <div
              className={`transition-transform duration-300 ease-in-out absolute ${isChatWidgetOpen ? "rotate-0 scale-100" : "-rotate-180 scale-0"}`}
            >
