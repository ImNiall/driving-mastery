...
import { Check, X, Sparkles, Crown, Calendar, Info, ShieldCheck } from "lucide-react";
import Seo from "./Seo";
...
const Badge: React.FC<{ className?: string; children: React.ReactNode }> = ({ children, className = "" }) => (
  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${className}` }>{children}</span>
);

const Feature: React.FC<{ ok?: boolean; children: React.ReactNode }> = ({ ok = true, children }) => {
  ...
  return (
    ...
    >
      {ok ? (
        <Check className={`mt-0.5 h-4 w-4 transition ${hover ? "text-indigo-700" : ""}` } />
      ) : (
  ...
  return (
    <div className="absolute top-4 right-4 z-10 hidden group-hover:flex">
      <div className="rounded-lg border bg-white/95 shadow-md backdrop-blur p-3 text-xs w-64">
        ...
        {items.length > 3 && <p className="mt-2 text-[11px] text-gray-500">…and more</p>}
      </div>
    </div>
  );
}
    ...
    <span className="relative inline-flex items-center">
      ...
      {open && (
        <div className="absolute left-0 top-6 z-20 w-80 rounded-lg border bg-white shadow-lg p-3 text-xs text-gray-700">
          <p className="font-semibold mb-1">Pass First Time – key terms (preview)</p>
          ...
          <p className="mt-2 text-[11px]">See full details on the Terms page.</p>
        </div>
      )}
    </span>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        {highlight ? <Crown className="h-5 w-5" /> : <Sparkles className="h-5 w-5" />}
      ...
      {highlight && (
        <div className="mt-2 text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-md p-2">
          ...
          <p className="mt-1 text-[11px] leading-snug text-emerald-800">
            Get a full refund if you don’t pass on your first attempt. <GuaranteeTooltip />.
          </p>
        </div>
      )}
    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-indigo-50 via-white to-white px-4 py-10">
      ...
      <div className="mx-auto max-w-6xl">
        <header className="text-center">
          <Badge className="bg-indigo-100 text-indigo-700">Stage 1 · Theory Only</Badge>
          <h1 className="mt-3 text-3xl md:text-4xl font-extrabold tracking-tight">Choose your plan</h1>
          <p className="mt-2 text-gray-600 max-w-2xl mx-auto">
            Start free, upgrade to unlock modules, full insights and unlimited AI coaching.
          </p>
        </header>
        ...
        <div className="mt-4 mx-auto max-w-2xl rounded-xl border bg-white/60 backdrop-blur p-3 flex items-center justify-center gap-2 text-sm text-emerald-800">
          ...
          <span>
            <strong>Guarantee:</strong> Pass first time or get a full refund (<GuaranteeTooltip />)
          </span>
        </div>

        {/* Cards */}
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-5" role="radiogroup" aria-label="Pricing plans">
          {/* Free */}
          <Card
            title="Starter"
            price="£0"
          ...
          >
            ...
            <div className="mt-4 rounded-lg bg-gray-50 p-3 text-xs text-gray-600">
              ...
              <div className="mt-1">Modules & AI locked on Free. Upgrade for full access.</div>
            </div>
          </Card>
          ...
          >
            ...
            <div className="mt-4 rounded-lg bg-indigo-50 p-3 text-xs text-indigo-900">
              ...
              <div className="mt-1">Modules, insights, and unlimited AI coaching included.</div>
            </div>
          </Card>
        </div>
        ...
        <p className="mt-6 text-xs text-center text-gray-500">
          ...
          <br />
          <strong>Pass First Time Guarantee:</strong> Full refund if you don’t pass on your first attempt — detailed Terms & Conditions apply.
        </p>
      </div>
    </div>
  );
