...
const ModulesViewV2: React.FC<ModulesViewProps> = ({ selectedModule, setSelectedModule }) => {
  // Minimal, clean rebuild: list -> detail stub
      ...
      <ul className="mt-4 divide-y divide-gray-100">
        {LEARNING_MODULES.map(m => (
          <li key={String(m.slug)} className="py-4 flex items-start justify-between">
            <div className="pr-4">
              <span className="text-xs font-semibold bg-brand-blue-light text-brand-blue py-1 px-2 rounded-full"><SafeText value={m.category} /></span>
              <h3 className="text-lg font-bold text-gray-800 mt-2"><SafeText value={m.title} /></h3>
              <p className="text-sm text-gray-600 mt-1"><SafeText value={m.summary} /></p>
            </div>
            <button
