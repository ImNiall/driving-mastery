        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
            <div className="bg-white p-6 md:p-8 rounded-lg shadow-xl">
                <div className="text-center">
                    {passed ? <TrophyIcon className="w-16 h-16 text-yellow-400 mx-auto" /> : <ClipboardListIcon className="w-16 h-16 text-blue-500 mx-auto" />}
                    <h2 className="text-3xl font-bold text-gray-800 mt-4">{passed ? "Congratulations, you passed!" : "Good Effort, Keep Practicing!"}</h2>
                    <p className="text-lg text-gray-600 mt-2">You scored:</p>
                    <p className={`text-7xl font-bold my-4 ${passed ? 'text-brand-green' : 'text-brand-red'}`}>{percentage}%</p>
                    <p className="text-gray-500 text-lg">({totalCorrect} out of {totalQuestions} correct)</p>
                </div>
                
                {pointsEarned > 0 && (
                    <div className="mt-6 pt-6 border-t border-gray-200 text-center">
                        <h3 className="text-lg font-semibold text-gray-700">Mastery Points Earned</h3>
                        <p className="text-4xl font-bold text-yellow-500 my-2">+{pointsEarned.toLocaleString()} MP</p>
                        <button 
            ...
            {weakestTopics.length > 0 && (
                <div className="bg-white p-6 rounded-lg shadow-lg">
                    <h3 className="text-xl font-bold text-gray-800 mb-4 text-center">Recommended Study</h3>
                    <div className="space-y-3">
                        {weakestTopics.map(topic => (
                            <div key={topic.category} className="bg-slate-50 p-4 rounded-lg flex justify-between items-center">
                                <div>
                                    <p className="font-bold text-gray-700">{topic.category}</p>
                                    <p className="text-sm text-red-600 font-semibold">Your score: {Math.round(topic.correct / topic.total * 100)}%</p>
                                </div>
                                <button
            ...
            <div className="mt-8 pt-6">
                <h3 className="text-2xl font-bold text-gray-800 text-center mb-6">Review Your Answers</h3>
                <div className="space-y-6">
                    {questions.map((q, index) => {
                        const userAnswer = finalUserAnswers.find(a => a.questionId === q.id);
                        return (
                            <div key={q.id}>
                                <p className="font-bold text-gray-700 mb-2">Question {index + 1}</p>
                                <QuestionCard
                                    question={q}
                                    isReviewMode={true}
