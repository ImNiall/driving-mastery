...
const FloatingChatWidget: React.FC<FloatingChatWidgetProps> = ({ isOpen, onClose, onStartCustomQuiz, initialContextMessage }) => {
            ...
            {!isInitialized.current && isLoading ? (
                <div className="flex justify-start">
                    <div className="bg-gray-200 text-gray-800 px-3 py-2 rounded-lg">
                        <span className="animate-pulse text-sm">AI Mentor is getting ready...</span>
                    </div>
                </div>
            ) : messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs px-3 py-2 rounded-lg ${msg.role === 'user' ? 'bg-brand-blue text-white' : 'bg-gray-200 text-gray-800'}`}>
