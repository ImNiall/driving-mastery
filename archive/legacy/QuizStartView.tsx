...
const QuizStartView: React.FC<QuizStartViewProps> = ({
  categories,
  onStartQuiz,
  onBack,
  quizHistory,
  ...
  const options = [
    {
      ...
      description: "A brief 10-question quiz to quickly assess your knowledge.",
    },
    {
      length: 25,
      title: "Standard Mock Test",
      description:
        "A comprehensive 25-question practice test covering a range of topics.",
    },
    {
      length: 50,
      title: "Official Mock Test",
      description:
        "A full-length 50-question test that mirrors the format of the official DVSA exam.",
    },
  ];

  const getStatsForLength = (length: number) => {
    const attemptsForLength = quizHistory.filter(
      (attempt) => attempt.length === length,
    );
    const totalAttempts = attemptsForLength.length;
    if (totalAttempts === 0) {
      return { attempts: 0, averageScore: 0 };
    }
    const totalScore = attemptsForLength.reduce(
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in">
      <button onClick={onBack} className="text-brand-blue font-semibold mb-6">
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {attempt.length} Questions
                      </p>
                    </div>
                    <div className="text-right">
                      <p
                        className={`font-bold text-lg ${percentage >= 86 ? "text-brand-green" : "text-brand-red"}`}
                      >
                        {percentage}%
                      </p>
                      <p className="text-sm text-gray-500">
