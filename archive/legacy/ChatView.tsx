...
const ChatView: React.FC<ChatViewProps> = ({ onStartCustomQuiz }) => {
  ...
  useEffect(() => {
    ...
    const initializeChat = async () => {
      ...
      const { text, action } = await getChatResponse(
        [initialUserMessage],
        token,
      );
        ...
        <div className="space-y-4">
          {messages.length === 0 && isLoading
            ...
            : messages.map((msg, index) => (
                ...
                >
                  <div
                    className={`max-w-xs md:max-w-md lg:max-w-lg px-4 py-2 rounded-xl ${msg.role === "user" ? "bg-brand-blue text-white" : "bg-gray-200 text-gray-800"}`}
                  >
                    {msg.text}
                    {msg.action?.type === "start_quiz" && (
                      ...
                      >
                        Start {msg.action.questionCount}-Question Quiz on{" "}
                        {msg.action.categories.join(", ")}
                      </button>
                    )}
                  </div>
                </div>
