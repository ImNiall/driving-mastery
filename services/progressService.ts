...
export async function getProgress(token?: string, category?: string): Promise<ProgressItem[]> {
...
export async function deleteProgress(token?: string, category?: string): Promise<void> {
  const url = category ? `${API_URL}?category=${encodeURIComponent(category)}` : API_URL;
