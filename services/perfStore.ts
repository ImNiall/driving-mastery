...
export function appendQuizResult(results: FinalQuizResults) {
  ...
  for (const k of Object.keys(byCategory)) {
    ...
    catStats[k] = { correct: prev.correct + byCategory[k].correct, total: prev.total + byCategory[k].total };
  }

  const percentage = results.totalQuestions ? Math.round((results.totalCorrect / results.totalQuestions) * 100) : 0;
  history.unshift({ ts: Date.now(), total: results.totalQuestions, correct: results.totalCorrect, percentage, byCategory, flaggedCount: results.flaggedQuestions.length });
  writeJSON(LS_QUIZ_HISTORY, history.slice(0, 50));
  writeJSON(LS_CATEGORY_STATS, catStats);
}

export function recordModuleProgress(moduleId: string, percent: number, name?: string) {
  const mp = readJSON<ModuleProgress>(LS_MODULE_PROGRESS, {});
  const next = Math.max(0, Math.min(100, Math.round(percent)));
  const prev = mp[moduleId]?.percent ?? 0;
  mp[moduleId] = { name: mp[moduleId]?.name ?? name, percent: Math.max(prev, next) };
...
export function getPerformanceSnapshot(): PerformanceSnapshot {
  ...
  const avgScore = attempts ? Math.round(history.reduce((s,h)=>s+h.percentage,0)/attempts) : 0;
  const recentScore = history[0]?.percentage;
  return { summary: { attempts, avgScore, recentScore }, categoryStats: catStats, moduleProgress, recentQuizzes: history };
}
